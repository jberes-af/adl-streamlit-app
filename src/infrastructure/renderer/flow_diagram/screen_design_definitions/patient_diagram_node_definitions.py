# /src/infrastructure/renderer/flow_diagram/patient_diagram_node_definitions.py
...
PATIENT_SUMMARY_NODE = FlowNode(
    ...
    screen_images=tuple([
        "patient_view/patient_summary.png",
        "patient_view/patient-summary-option-b.png",
    ]),
    required_permissions=tuple([
        PermissionRequirement(
            domain=DomainObject.ADL_ASSESSMENT,
...
PATIENT_EDGE_1 = FlowEdge(
    source="ADL_Home",
    target="Patient_Summary",
    action_label=None
)

PATIENT_EDGE_2 = FlowEdge(
    source="Patient_Summary",
    target="ADL_Entry",
    action_label="Tap Enter ADL"
)

PATIENT_EDGE_3 = FlowEdge(
    source="ADL_Entry",
    target="Patient_Summary",
    action_label="Save")

PATIENT_EDGE_4 = FlowEdge(
    source="Patient_Summary",
    target="OBS_Entry",
    action_label="Tap Add Observation"
)

PATIENT_EDGE_5 = FlowEdge(
    source="OBS_Entry",
    target="Patient_Summary",
    action_label="Save"
)

PATIENT_EDGE_6 = FlowEdge(
    source="Patient_Summary",
    target="Priority",
    action_label="Tap Prioritize"
)

PATIENT_EDGE_7 = FlowEdge(
    source="Priority",
    target="Patient_Summary",
    action_label="Submit"
)

PATIENT_EDGE_8 = FlowEdge(
    source="Patient_Summary",
    target="History",
    action_label="Tap View History"
)

PATIENT_EDGE_9 = FlowEdge(
    source="History",
    target="ADL_Home",
    action_label="Back"
)
